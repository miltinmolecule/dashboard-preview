      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* <AppInput
            ...
            inputClass="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 w-36"
            value={search}
          ...
          <thead>
            <tr className="border-b border-gray-100">
              {[
                ...
                <th
                  key={h}
                  className="pb-3 text-left text-xs font-semibold text-gray-500 pr-4"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.map((ride) => (
              <tr key={ride.id} className="hover:bg-gray-50/50">
                <td className="py-3 pr-4 text-xs font-mono text-gray-700">
                  {ride.id}
                </td>
                ...
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-1">
                    {Array.from({ length: Math.min(ride.passengers, 3) }).map(
                      (_, i) => (
                        <div
                          key={i}
                          className="h-6 w-6 rounded-full bg-gray-200 border-2 border-white -ml-1 first:ml-0 flex items-center justify-center"
                        >
                          <svg
